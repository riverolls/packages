 ...
 * Generated class from Pigeon that represents data sent in messages.
 */
data class PlaybackInfoChangeEvent (
  /** The video duration in milliseconds. */
  val duration: Long,
  /** The width of the video in pixels. */
  val width: Long,
 ...
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): PlaybackInfoChangeEvent {
      val duration = pigeonVar_list[0] as Long
      val width = pigeonVar_list[1] as Long
      val height = pigeonVar_list[2] as Long
      val rotationCorrection = pigeonVar_list[3] as Long
      return PlaybackInfoChangeEvent(duration, width, height, rotationCorrection)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
  ...
  override fun equals(other: Any?): Boolean {
    if (other !is PlaybackInfoChangeEvent) {
      return false
    }
    if (this === other) {
      return true
...
private open class MessagesPigeonCodec : StandardMessageCodec() {
  override fun readValueOfType(type: Byte, buffer: ByteBuffer): Any? {
    return when (type) {
      ...
      132.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
          PlaybackInfoChangeEvent.fromList(it)
        }
      }
      133.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
...
interface VideoPlayerInstanceApi {
  ...
  fun setDataSource(options: CreationOptions)
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: VideoPlayerInstanceApi?, messageChannelSuffix: String = "") {
      val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.video_player_android.VideoPlayerInstanceApi.setDataSource$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val optionsArg = args[0] as CreationOptions
            val wrapped: List<Any?> = try {
              api.setDataSource(optionsArg)
              listOf(null)
            } catch (exception: Throwable) {
              MessagesPigeonUtils.wrapError(exception)
            }
